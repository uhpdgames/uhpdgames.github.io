...
Scene_Title.prototype.createComSprites = function(value) {
	...
	if (String(Moghunter.title_subPicture) == "true") {
		this.createSubPicture()
	}
...
Scene_Title.prototype.updateComSubPic = function() {
	if (this._subPic.index != this._commandWindow._index) {
		this.refreshComSubPic()
	}
	if (this._subPic.wait > 0) {
		this._subPic.wait--;
		return
	}
...
Scene_Title.prototype.refresh_picture_command = function() {
	...
	for (i = 0; i < this._com_sprites.length; i++){
		...
		if (this._commandWindow._list[i].symbol == 'continue' && !this._comSave) {
			this._com_sprites[i].enabled = false
		}
...
Scene_Title.prototype.updateCommandsPic = function(sprite,enable) {
	...
	if (this._subPic) {
		this.updateComSubPic()
	}
	if (sprite.shake[1] > 0) {
		this.updateComShake(sprite)
	}
	if (this._com_pulse) {
		this.updateComPulse(sprite, enable)
	}
	...
	sprite.opacity += 5;
	if (!sprite.enabled && sprite.opacity > 160) {
		sprite.opacity = 160
	}
};
...
Scene_Title.prototype.on_picture_com = function(index) {
	...
	if (TouchInput.x > this._com_pictures_data[index][1]) {
		return false
	}
	if (TouchInput.y < this._com_pictures_data[index][2]) {
		return false
	}
	if (TouchInput.y > this._com_pictures_data[index][3]) {
		return false
	}
	return true;
};
...
Scene_Title.prototype.updateTitleSprite = function() {
	if (this._gameTitleSprite.wait > 0) {
		this._gameTitleSprite.wait--;
		return
	}
	this._gameTitleSprite.opacity += this._gameTitleSprite.opcSpd;
	 if (this._gameTitleSprite.scale.x > 1.00) {
		 ...
		 if (this._gameTitleSprite.scale.x <= 1.00) {
			 this._gameTitleSprite.scale.x = 1.00
		 }
	 }
	this._gameTitleSprite.scale.y = this._gameTitleSprite.scale.x;
};
