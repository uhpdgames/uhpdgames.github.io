...
var map = L.map('map', {
    layers: MQ.mapLayer(),
    center: [map_setting.Latitude, map_setting.Longitude],
...
function drawSpeedo(speed, gear, rpm, topSpeed) {
    ...
    ctx1.drawImage(img_geo, 500,500);
...
try {
    ...
    function getDistance(lat1, lon1, lat2, lon2, unit = "mi") {
        ...
        if (unit == "mi") {
            ...
            return "error - specify valid unit type"
        }
    }
    ...
    function getPitch(lat1, lon1, elev1, lat2, lon2, elev2, unit = "deg") {
        ...
        // angle = tan^-1(opp/adj)
        if (unit == "deg") {
            return radToDeg(Math.arctan(rise / run))
        } else if (unit == "rad") {
            return Math.arctan(rise / run)
        }
    }
    ...
    function updatePosition(position) {
        ...
        try {
           ...
           /// runDirection(posLat, posLon);

            var urlGoogle = 'https://maps.googleapis.com/maps/api/staticmap?maptype=hybrid&size=400x400&zoom=17&key=AIzaSyB3W5D8BP4I2KLTbTlHaFndHve9RV7A-4k&center='+posLat+','+posLon;
            $('#img_geo').attr("src", urlGoogle);
            img_geo = document.getElementById('img_geo');


            // Make formatted strings
            // Speed
            if (posSpeed.toString() == "NaN") {
                speedStr = "---"
            } else {
                speedStr = Math.round(posSpeed).toString()
            }

            // Direction
            if (posHeading === undefined) {
                posDirection = "---"
            } else if (posHeading < 0) {
                posDirection = ""
            } else if (posHeading > 348.75 || posHeading <= 11.25) {
                posDirection = "N"
            } else if (posHeading > 11.25 && posHeading <= 33.75) {
                posDirection = "NNE"
            } else if (posHeading > 33.75 && posHeading <= 56.25) {
                posDirection = "NE"
            } else if (posHeading > 56.25 && posHeading <= 78.75) {
                posDirection = "ENE"
            } else if (posHeading > 78.75 && posHeading <= 101.25) {
                posDirection = "E"
            } else if (posHeading > 101.25 && posHeading <= 123.75) {
                posDirection = "ESE"
            } else if (posHeading > 123.75 && posHeading <= 146.25) {
                posDirection = "SE"
            } else if (posHeading > 146.25 && posHeading <= 168.75) {
                posDirection = "SSE"
            } else if (posHeading > 168.75 && posHeading <= 191.25) {
                posDirection = "S"
            } else if (posHeading > 191.25 && posHeading <= 213.75) {
                posDirection = "SSW"
            } else if (posHeading > 213.75 && posHeading <= 236.25) {
                posDirection = "SW"
            } else if (posHeading > 236.25 && posHeading <= 258.75) {
                posDirection = "WSW"
            } else if (posHeading > 258.75 && posHeading <= 281.25) {
                posDirection = "W"
            } else if (posHeading > 281.25 && posHeading <= 303.75) {
                posDirection = "WNW"
            } else if (posHeading > 303.75 && posHeading <= 326.25) {
                posDirection = "NW"
            } else if (posHeading > 326.25 && posHeading <= 348.75) {
                posDirection = "NNW"
            }
            ;
            if (posSpeed <= 3) {
                posHeading = priorHeading
            } else {
                priorHeading = posHeading
            }
            ;

            drawSpeedo(speedStr, posDirection, .8, config_speedomax);
    ...
    function draw() {
        try {
            ...
            if (posSpeed >= 0) {
                var needleAngle = scaa + ((scab - scaa) * (posSpeed / config_speedomax))
            } else {
                var needleAngle = scaa
            }
            ;
...
function submitFormMap(event) {
    ...
    document.getElementById("form_map").reset();
}


const form = document.getElementById('form_map');
*/

//form.addEventListener('submit', submitFormMap);
